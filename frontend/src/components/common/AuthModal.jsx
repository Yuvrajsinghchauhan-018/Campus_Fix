...
const AuthModal = ({ isOpen, onClose, role }) => {
  ...
  // Force strict unmounting of ghost state errors across modal open/closes
  useEffect(() => {
    if (isOpen) {
      setError('');
      setMsg('');
  ...
  // Authorities CAN register now, so canRegister is always true for them as well
        ...
        >
          ...
          >
            ...
            <p className="text-slate-400 text-center text-sm mb-8 font-medium">
              {!showSuccess
                ? (role === 'authority' ? 'Secure access for authorized personnel.' : 'Login or create your account to continue.')
                : 'Account Status'}
            </p>
            ...
            {error && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 p-4 bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl text-sm font-medium leading-relaxed">
                ...
                {role === 'maintainer' && error.includes('Ask authority') && (
                  <button type="button" onClick={() => { setTab('register'); setError(''); }} className="mt-4 bg-white text-slate-900 border border-slate-200 px-5 py-2.5 rounded-lg font-bold text-xs uppercase tracking-widest shadow-sm hover:scale-105 active:scale-95 transition-all w-full">
                    Request Access
                  </button>
                )}
              </motion.div>
                ...
                {tab === 'register' ? (
                  ...
                  >
                    ...
                    {role === 'authority' && (
                      <>
                        <div className="space-y-1.5">
                          <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 ml-1">Admin Secret Key</label>
                          <input type="password" name="adminSecretKey" required value={formData.adminSecretKey} onChange={handleChange} className="input-field py-3 bg-slate-800/40 border-slate-700/50 focus:bg-slate-800 text-sm" placeholder="Enter Registration Key" />
                        </div>
                        <div className="space-y-1.5">
                          <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 ml-1">Block / Building</label>
                          <select name="block" required value={formData.block} onChange={handleChange} className="input-field py-3 bg-slate-800/40 border-slate-700/50 focus:bg-slate-800 text-sm">
                        ...
                        <div className="space-y-2 md:col-span-2 mt-2">
                          <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 ml-1">Floors Incharge</label>
                        ...
                        <div className="space-y-2 md:col-span-2 mt-2">
                          <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 ml-1">Responsibilities</label>
                          <div className="flex flex-wrap gap-2">
                    ...
                    {role === 'authority' && (
                      <div className="space-y-1.5">
                        <div className="flex justify-between items-center mb-1">
                          <label className="text-xs font-bold uppercase tracking-wider text-slate-400 ml-1">Admin Secret Key</label>
                        </div>
                        <input type="password" name="adminSecretKey" required value={formData.adminSecretKey} onChange={handleChange} className="input-field py-3.5 bg-slate-800/40 border-slate-700/50 focus:bg-slate-800 text-sm" placeholder="Enter Registration Key" />
                      </div>
                    )}
